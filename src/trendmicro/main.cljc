(ns trendmicro.main
  "Kotodama WASM entrypoint for the Trendmicro clean-room actor (L4) — Clojure port.
  ...
  facts (`trendmicro.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "trendmicro")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"trendmicro.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "trendmicro-compat" :tier tier :entities entities} 200])
...
(defn start! [] :trendmicro-compat/ready)
